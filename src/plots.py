"""
File used to generate plots from bootstrap results 
...
size = 0.05 
# Define bin edges
bin_edges = [i * size for i in range(int(data['comparisons_fraction'].min() / size), int(data['comparisons_fraction'].max() / size) + 2)]
...
grouped_df = data.groupby('bin').agg({'f1_score': 'mean','f1*_score': 'mean','pair_quality': 'mean','pair_completeness': 'mean','comparisons_fraction': 'median'})
